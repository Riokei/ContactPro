...
namespace ContactPro.Services
{
    public class DataGenService : IDataGen
    {
        ...
        public async Task<List<Contact>> GenerateContacts(AppUser User)
        {
            ...
            {
                    //TO-DO
                    //1. Change images to something not provided by coder foundry.
                    //2. Make more contacts, set some contacts to categories.
                    //3. See about how to test the email properly...
                new Contact() {AppUserId = User.Id,FirstName = "John", LastName = "Smith", Address1 = "1234 Apple Street", BirthDate = new DateTime(1994, 08, 16), City = "Paris", State = Enums.States.TX, Email = "test@test.com", ZipCode = 21000, PhoneNumber = "0000000000", ImageData = File.ReadAllBytes(Path.Combine(currentDirectory, "wwwroot\\img\\ClaudiaBlack.png")), ImageType = ".png" },

                new Contact() {AppUserId = User.Id, FirstName = "Eric", LastName = "Doe", Address1 = "11 Dell Place", BirthDate = new DateTime(1976, 01, 02), City = "Roanoke", State = Enums.States.VA, Email = "test@test.com", ZipCode = 18701, PhoneNumber = "0000000000", ImageData = File.ReadAllBytes(Path.Combine(currentDirectory, "wwwroot\\img\\NeilKaplan.png")), ImageType = ".png" }
            };
            }
